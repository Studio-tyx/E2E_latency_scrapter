...
def actuate_func(func_id):
    print(f'call: {func_id}')
    res = requests.get('http://10.214.131.192:9211/func/{}'.format(func_id))
    return res.text
...
def callback(ch, method, properties, body):
    ...
    rules = []
    exec_threads = []
    print(num_of_related_rules)
    print(key)
    for i in range(num_of_related_rules):
        rid = r.lindex(key, i)     # rules集合中的某条rule
        rule = r.hgetall(str(rid))
        t = threading.Thread(target=check_valid, args=(key, rule,))
        check_threads.append(t)
        rules.append(rule)
        t.start()
    for i in range(num_of_related_rules):
        check_threads[i].join()
        if check_threads[i].get_result() == True:
            actions = rules[i]["Actions"].split(',')
            for action in actions:
                md5encoder = hashlib.new('md5')
                func_id = md5encoder.update(action.encode('utf-8'))
                t = threading.Thread(target=actuate_func, args=(func_id,))
